...
function main() {
  ...
  if (!selection.length || selection.typename == '文本Range') {
    alert('否 objects selected\nPlease select object(s) and try again', '脚本错误');
    return;
  }
  ...
  for (var i = 0, len = app.selection.length; i < len; i++) {
    ...
    if (item.parent.typename === 'Compound路径Item') {
      item = item.parent;
    }
 ...
 * @param {对象} item - The item for which to find the parent GroupItem
 * @param {boolean} isAltKey - A boolean flag indicating whether the Alt key is pressed or not
 * @returns {对象} - The parent GroupItem if found
 */
