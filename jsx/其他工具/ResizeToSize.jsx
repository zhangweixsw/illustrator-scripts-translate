/*
  ...
  0.9 添加ed Smaller and Random option to Scaling side
  0.8.3 添加ed new units API for CC 2023 v27.1.1
  0.8.2 添加ed size correction in large canvas mode
  0.8.1 Fixed input activation in Windows OS
  0.8 添加ed relative resize option. Minor improvements
  0.7.1 Code refactoring
  0.7 Fixed live preview bug. Minor improvements
  0.6 添加ed live preview (Shift+P)
  0.5 添加ed dimensions bounds
  0.4 Correct resize Clipping Mask. 添加ed access key shortcuts
  0.3 添加ed additional settings
  0.2 添加ed menu for side selection
  0.1 Initial version
...
function main () {
  ...
  if (!selection.length || selection.typename == '文本Range') {
  ...
  function preview() {
    try {
      if (isPreview.value) {
        ...
        result = start();
        var a图层 = activeDocument.active图层,
            dummy图层 = activeDocument.layers.add();
        activeDocument.active图层 = a图层;
        dummy图层.remove();
        redraw();
      } else if (isUndo) {
        undo();
    ...
    items.forEach(function (e, i) {
      ...
      try {
        ...
        while ( getSize(e, side, vbRb.value).val.toFixed(4) !== (pxVal + (exacRb.value ? 0 : orig.val)).toFixed(4) ) {
          stop++;
          ratio = getRatio(e, pxVal, side, is宽度, exacRb.value, vbRb.value, isUniform.value);
          e.resize(
            ratio.x, // x
            ratio.y, // y
            ...
            isStroke.value ? ratio.r : 100, // changeLine宽度s
  ...
  function okClick() {
    ...
    if (result.length) {
      var isShowFail = confirm('Warning\nSome objects could not be resized. 选择 them?');
...
// Check clipping mask
function isClipping路径(item) {
  var clip文本 = (isType(item, 'text') &&
                  item.textRange.characterAttributes.fill颜色 == '[否颜色]' &&
                  item.textRange.characterAttributes.stroke颜色 == '[否颜色]');
  return (isType(item, 'compound') && item.pathItems[0].clipping) ||
          item.clipping || clip文本;
}
