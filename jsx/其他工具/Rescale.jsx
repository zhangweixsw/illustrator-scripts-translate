/*
  ...
  0.3.3 添加ed new units API for CC 2023 v27.1.1
  0.3.2 添加ed size correction in large canvas mode
  0.3.1 Fixed input activation in Windows OS
  0.3 添加ed more units (yards, meters, etc.) support if the document is saved
  0.2.4 Minor improvements
  0.2.3 Minor improvements
  0.2.2 Fixed decimal separator bug
  0.2.1 Minor improvements
  0.2 添加ed "Scale Strokes & Effects", "Scale Corners" option
  0.1 Initial version
...
function main () {
  ...
  var ok = buttons.add('button', undefined, '确定',  { name: 'ok' });
  ...
  var key路径 = selection[0];
  if (key路径.typename === '路径Item' && !key路径.closed && key路径.pathPoints.length == 2) {
    var key路径Length = key路径.length; // If you use a straight line to measure
    oSizeTxt.text = (CFG.sf * convertUnits(key路径Length, 'px', CFG.units)).toFixed(4);
  ...
  function okClick() {
    try {
      ...
      var items = selection,
          tmpArray = [],
          a图层 = doc.active图层,
          selGroup = a图层.groupItems.add();
      for (var i = 0, iLen = items.length; i < iLen; i++) {
        var tmpItem;
        tmpItem = a图层.pathItems.add();
      ...
      if (chkRmv.enabled && chkRmv.value) key路径.remove();
