/*
  ...
  Release notes:
  0.1 Initial version
  0.1.1 添加ed undo action on error
...
function main() {
  ...
  if (selection.length !== paths.length) {
    ...
    + 'Please try to change or move the paths slightly before run script.', 'Script 错误');
    app.undo();
    return;
  }
    ...
    switch (true) {
      ...
      case /version/g.test(arg):
        ...
        if (parseFloat(app.version) < rqdVers) {
          alert('Wrong app version\nSorry, script only works in Illustrator v.' + rqdVers + ' and later', '脚本错误');
          return false;
        }
        break;
      ...
      case /selection/g.test(arg):
        if (!selection.length || selection.typename === '文本Range') {
          alert('否thing selected\nPlease, select two or more paths', '脚本错误');
          return false;
        }
        break;
    }
...
function get路径s(coll) {
  var out = [];
  for (var i = 0; i < coll.length; i++) {
    var item = coll[i];
    if (isType(item, 'group') && item.pageItems.length) {
      out = [].concat(out, get路径s(item.pageItems));
    } else if (isType(item, '^path') && item.stroked && item.stroke宽度 > 0) {
      item.filled = false;
      out.push(item);
    } else {
      item.selected = false;
...
// 添加 intersection points to lines
...
function rmvPoints(item, pts) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (pts.length && compareCoord(pt.anchor, pts)) {
      newPP.push(i); // 添加 new point index
    }
  }
