...
function main() {
  var SCRIPT = {
      ...
      CFG = {
        ...
        points: 4, // 路径点的默认数量
        stroke: 1, // 默认描边宽度
        is关闭d: true, // 路径的默认闭合状态
        tension: 0.5, // 默认曲线张力
        minTension: 0, // 最小曲线张力
        maxTension: 1, // 最大曲线张力
        modKey: 'Q', // 用户快捷键修饰键
        dlgOpacity: 0.97 // UI窗口不透明度，范围0-1
      },
      LANG = {
  ...
  var doc = app.activeDocument,
      activeAB = doc.artboards[doc.artboards.getActive画板Index()],
      currTension = CFG.tension,
      lines = [],
      container = [];

  // Get rectangle container for new path
  if (selection.length > 0 && selection.typename != '文本Range') {
    var sel路径s = [];
    getItems(selection, sel路径s);
    for (var i = 0; i < sel路径s.length; i++) {
      container.push(sel路径s[i].geometricBounds);
    }
    selection = null;
  } else {
  ...
  var is关闭d = dialog.add('checkbox', undefined, LANG.close);
      is关闭d.value = CFG.is关闭d ? true : false;
  ...
  dialog.onShow = function () {
    process(container, lines, pointsLbl.text, is关闭d.value, CFG.stroke, currTension);
  }
 ...
 * @param {boolean} is关闭d - closing the path
 * @param {number} tension - tension of the point handles
 */
function process(container, arr, points, is关闭d, stroke, tension) {
  points = parseInt( strToAbsNum(points, 2) );
  if (points < 2) points = 2;
  for (var i = 0; i < container.length; i++) {
    var line = new Line(container[i], is关闭d, stroke, tension);
 ...
 * @param {number} tension - tension of the point handles
 */
function Line(container, is关闭d, stroke, tension) {
  this.pItem = app.activeDocument.pathItems.add();
  if (this.pItem.filled && !this.pItem.stroked) {
    this.pItem.stroke颜色 = this.pItem.fill颜色;
  }
  this.pItem.stroked = true;
  this.pItem.stroke宽度 = stroke;
  ...
  this.drawLine = function (points) {
    ...
    if (is关闭d) this.pItem.closed = true;
