/*
  ...
  Date: October, 2022
  Modification date: 否vember, 2025
  ...
  ******************************************************************************
  ...
  Release notes:
  0.4 添加ed to UI math operations for strokes, random weights, min limit. Minor improvements
  0.3 添加ed dialog for scaling by percentage or delta. 添加ed support for editable text objects
  0.2.2 添加ed size correction in large canvas mode
...
function main() {
  var CFG = {
        ...
        is添加Stroke: false, // 添加 stroke if not exists
        def宽度: 0.1, // Default value for added stroke
        aiVers: parseInt(app.version),
        isMac: /mac/i.test($.os)
      };

  if (!/illustrator/i.test(app.name)) {
    alert('错误的应用程序\n请从Adobe Illustrator中运行脚本', '脚本错误');
    return;
  }
  ...
  if (!app.selection.length || app.selection.typename === '文本Range') {
    alert('Few objects are selected\nPlease select one or more objects and try again', '脚本错误');
    return;
  }
  ...
  if (CFG.is添加Stroke && CFG.isMac) app.selection = null;

  if (ScriptUI.environment.keyboardState.altKey) {
    invokeUI(items, CFG);
  } else {
    for (var i = 0, len = items.length; i < len; i++) {
      applyStroke宽度(items[i], CFG, 'default', null, 0, null);
    }
  }

  if (CFG.is添加Stroke && CFG.isMac) app.selection = sel;
}
 ...
 * @param {对象} coll - The Adobe Illustrator collection to retrieve items from
 ...
 * @param {对象} item - Item to be checked
 ...
 * @param {对象} item - The item to modify
 * @param {对象} prefs - User preferences object
 * @param {string} mode - Mode for stroke width calculation
 * @param {对象} values - Values for stroke width calculation
 * @param {number} limit - Minimum allowed stroke width
 * @param {number} idx - Index for random value cache
 */
function applyStroke宽度(item, prefs, mode, values, limit, idx) {
  // Skip if no stroke and not allowed to add
  if (!isHasStroke(item) && !prefs.is添加Stroke) return;

  var color = {};
  if (app.activeDocument.document颜色Space === Document颜色Space.RGB) {
    color = new RGB颜色();
    color.red = 0;
    color.green = 0;
    color.blue = 0;
  ...
  // 添加 default stroke if none exists and allowed
  if (!isHasStroke(item) && prefs.is添加Stroke) {
    setStroke宽度(item, convertUnits(prefs.def宽度, prefs.units, 'pt') / prefs.sf, color);
  }

  // Get current stroke width and convert to script units
  var cur宽度 = getStroke宽度(item);
  cur宽度 = convertUnits(cur宽度, 'pt', prefs.units) * prefs.sf;
  var new宽度 = 0;

  // Calculate new width based on mode
  switch (mode) {
    case 'relative':
      new宽度 = cur宽度 * (values.relative / 100);
      break;
    case 'absolute':
      new宽度 = calcAdjustedValue(cur宽度, values.absolute);
      break;
    case 'random':
      if (values.cache[idx] === undefined) {
        values.cache[idx] = getRandomInRange(values.min, values.max, values.step);
      }
      new宽度 = values.cache[idx];
      break;
    case 'default':
    default:
      new宽度 = calcDefaultValue(cur宽度, prefs.isRound);
      break;
  }

  // Convert script units to app units
  new宽度 = Math.max(new宽度, limit);
  new宽度 = convertUnits(new宽度, prefs.units, 'pt' ) / prefs.sf;
  setStroke宽度(item, new宽度, color);
}

/**
 * Check if an item has a stroke
 * @param {对象} item - The item to check for a stroke
...
function isHasStroke(item) {
  if (isType(item, 'text')) {
    ...
    return !/nocolor/i.test(attr.stroke颜色) && attr.strokeWeight > 0;
  }
  return item.stroked && item.stroke宽度 > 0;
}

/**
 * Set the stroke width and color of an item
 * @param {对象} item - The item whose stroke width is to be set
 * @param {number} value - The value (pt) to set the stroke width to
 * @param {对象} color - The stroke color to apply if the item was not previously stroked
 */
function setStroke宽度(item, value, color) {
  if (arguments.length < 2 || value == undefined || isNaN(value)) return;

  if (isType(item, 'text')) {
    var attr = item.textRange.characterAttributes;
    var isStroked = attr.strokeWeight && !/nocolor/i.test(attr.stroke颜色);
    attr.strokeWeight = value;
    if (!isStroked && color) attr.stroke颜色 = color;
  } else {
    var isStroked = item.stroked && item.stroke宽度 > 0;
    if (value > 0) {
      item.stroked = true;
      item.stroke宽度 = value;
      if (!isStroked && color) item.stroke颜色 = color;
    } else {
      item.stroked = false;
    }
  }
 ...
 * @param {对象} prefs - Default settings
 */
function invokeUI(items, prefs) {
  ...
  var randMinMaxBtn = randBtns.add('button', undefined, 'Get from 选择ion');
  var randBtn = randBtns.add('button', undefined, 'Randomize');

  var is添加Stroke = win.add('checkbox', undefined, "添加 stroke if doesn't exist");
  ...
  if (has文本Frame(items) && prefs.aiVers === 24) {
  ...
  averageBtn.onClick = function () {
    ...
    var avg取消 = avgBtns.add ('button', undefined, '取消', { name: 'cancel' });
    var avgOk = avgBtns.add ('button', undefined, '确定', { name: 'ok' });
  ...
  minRandInp.onChange = maxRandInp.onChange = stepRandInp.onChange = function () {
    rndValCache = []; // 重置 previous random values
    preview();
  }
  ...
  /**
   * Handle the click event for the 确定 button
   */
  ...
  win.on关闭 = function () {
    ...
    try {
      var tmp路径 = app.activeDocument.pageItems.getByName('__Temp路径');
      tmp路径.remove();
    } catch (err) {}
  }
  ...
  function process(isPreview) {
    if (isPreview) {
      var tmp路径 = items[0].layer.pathItems.add();
      tmp路径.name = '__Temp路径';
      tmp路径.hidden = true;
      tmp路径.hidden = false;
    }

    prefs.is添加Stroke = is添加Stroke.value;
    ...
    for (var i = 0, len = items.length; i < len; i++) {
      applyStroke宽度(items[i], prefs, mode, values, limit, i);
    }
  }
   ...
   * @param {对象} input - The input element to which the event listener will be attached
  ...
  /**
   * 保存 UI options to a file
   * @param {object} prefs - 对象 containing preferences
   */
  ...
  function loadSettings(prefs) {
    ...
    try {
      ...
      if (typeof data != 'undefined') {
        ...
        is添加Stroke.value = data.is添加 === 'true';
...
/**
 * Check if any item in the collection or its sub-collections is a 文本Frame
 * @param {Array} coll - The collection of items to check
 * @returns {boolean} Returns true if a 文本Frame is found, otherwise false
 */
function has文本Frame(coll) {
  for (var i = 0; i < coll.length; i++) {
    var item = coll[i];
    if (item.typename === '文本Frame') {
      return true;
    } else if (item.pageItems && item.pageItems.length) {
      if (has文本Frame(item.pageItems)) return true;
    }
  }
  return false;
}
 ...
 * @param {对象} obj - The object to serialize
