/*
  ...
  Original idea by Carlos Canto (否v 21, 2013)
  ...
  Release notes:
  0.2 添加ed user interface. New options. Updated algorithm
...
function main() {
  ...
  var lay = get编辑able图层(doc);

  // Create triangle color
  var triangle颜色;
  if (/rgb/i.test(doc.document颜色Space)) {
    triangle颜色 = new RGB颜色();
    triangle颜色.red = CFG.rgb[0];
    triangle颜色.green = CFG.rgb[1];
    triangle颜色.blue = CFG.rgb[2];
  } else {
    triangle颜色 = new CMYK颜色();
    triangle颜色.cyan = CFG.cmyk[0];
    triangle颜色.magenta = CFG.cmyk[1];
    triangle颜色.yellow = CFG.cmyk[2];
    triangle颜色.black = CFG.cmyk[3];
  }
  ...
  if (app.selection.length && /path/i.test(app.selection[0].typename)) {
    selPoints = get选择edPoints(app.selection[0]);
  }
  ...
  var is添加Info = colLeft.add('checkbox', undefined, '添加 Triangle Data As 文本');
      is添加Info.value = false;
  ...
  function drawTriangle() {
    ...
    triangle.setEntire路径([p1, p2, p3]);
    triangle.closed = true;
    triangle.stroked = false;
    triangle.filled = true;
    triangle.fill颜色 = triangle颜色;
  ...
  * @param {对象} item - The input element to which the event listener will be attached
  ...
  */
   ...
   * @param {对象} item - The input object containing the text field to validate
  ...
  function loadSettings(prefs) {
    ...
    try {
      ...
      if (typeof data != 'undefined') {
        ...
        is添加Info.value = data.legend === 'true';
      }
    } catch (err) {
      return;
    }
 ...
 * @param {对象} doc - The Illustrator document to search for editable layers
 * @returns {对象} - The first editable layer found or created
 */
function get编辑able图层(doc) {
  var activeLay = doc.active图层;
 ...
 * @param {对象} point - The point to check
 * @returns {boolean} - True if the point is selected, false otherwise
 */
function is选择ed(point) {
  return point.selected == 路径Point选择ion.ANCHORPOINT;
}
